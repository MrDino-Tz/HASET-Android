...
//        super.onCreate(state); getWindow().setFlags(android.view.WindowManager.LayoutParams.FLAG_SECURE, android.view.WindowManager.LayoutParams.FLAG_SECURE);
    ...
    private void renderQrCode(String uri) {
        ...
        new Thread(() -> {
            ...
            } catch (Exception error) { runOnUiThread(() -> { qrProgress.setVisibility(View.GONE); fail("Unable to generate the authenticator QR code."); }); }
        }).start();
    }
